...
# embeds
helpmessage = discord.Embed(title="Commands", description="***c!au*** - commands for au winnas, has extra arguemnts \n***c!morelike*** [word] - responds with '[word]? more like [word but edited]' \n***c!pdp*** - gives a random pdp song \n***c!admin*** - options for admins use c!admin help for more \n***c!echo*** - repeats what the user tells it to **only big c and use this command**", color=0x7e01e4)
helpmessage.set_footer(text="comands with extra arguments can be used to get more info out of with c![command] help (i.e. c!au help)")
auhelpmessage = discord.Embed(title="AU commands", description="***c!au random*** - gives a random AU winna with image and name (not coded yet) \n***c!au list*** - lists all au winnas \n***c!au submit*** - submit an au winna to the mods (not added yet)", color=0x7e01e4)
aulist = discord.Embed(title="au winnas", description=f"{auwinnas[0]}\n{auwinnas[1]}\n{auwinnas[2]}\n{auwinnas[3]}\n{auwinnas[4]}\n{auwinnas[5]}\n{auwinnas[6]}\n{auwinnas[7]}\n{auwinnas[8]}\n{auwinnas[9]}\n{auwinnas[10]}\n{auwinnas[11]}\n{auwinnas[12]}\n{auwinnas[13]}\n{auwinnas[14]}\n{auwinnas[15]}\n{auwinnas[16]}\n{auwinnas[17]}\n{auwinnas[18]}\n{auwinnas[19]}\n{auwinnas[20]}\n{auwinnas[21]}\n{auwinnas[22]}\n{auwinnas[23]}\n{auwinnas[24]}\n{auwinnas[25]}\n{auwinnas[26]}\n{auwinnas[27]}\n{auwinnas[28]}\n{auwinnas[29]}\n{auwinnas[30]}\n{auwinnas[31]}\n{auwinnas[32]}", color=0x00C400)
aulistnoemoji = discord.Embed(title="au winnas that dont have emojis", description="there are none yet, check back later or submite some with c!au submit")
adminhelpmessage = discord.Embed(title="Admin Commands", description="***c!admin pfp*** - change the bot's pfp **only big c and use this command** \n***c!admin status*** - change the bots status, arguements are as follows 'c!admin status [online/idle/dnd/offline] [playing/watching/listening/streaming] [name of activity i.e. Minecraft]\n***c!admin name [something]*** - change bot name")
...
async def on_message(message):
	...
	# add reaction buttons for c!au submit
	...
	if message.content.startswith(prefix):
		if message.author == client.user:
			...
			if (argslist[1] == "list"): # list all of the officially reconized au winnas
				try:
					if (argslist[2] == "yes" or argslist[2] == "y"):
						...
						await message.channel.send("fun fact: you can do c!au list <yes/no> to choose whether or not you want the list of aus with or without the emojis", embed=aulist)
				except IndexError:
					await message.channel.send("fun fact: you can do c!au list <yes/no> to choose whether or not you want the list of aus with or without the emojis", embed=aulist)
			...
			for x in message.mentions:
				morelike = morelike.replace(f"<@c{x.id}>",x.name.lower())
			...
			try:
				await message.channel.send(message.content[7:],files=images)
			except discord.errors.HTTPException:
